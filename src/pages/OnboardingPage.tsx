    ...
    <div className="min-h-screen bg-[#f6f6f8] font-['Inter',sans-serif]">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 md:px-40">
        <div className="flex items-center gap-3">
          <div className="size-8 bg-[#1c43a6] rounded-lg flex items-center justify-center text-white">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center justify-center rounded-lg h-10 px-4 bg-[#1c43a6] text-white text-sm font-bold hover:bg-[#1c43a6]/90 transition-all"
          >
            Đăng xuất
          </button>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div key={i} className="contents">
                  <div className={`flex flex-col items-center gap-2 transition-all ${i <= currentStep ? '' : 'opacity-50'}`}>
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-all ${
                      i <= currentStep
                        ? 'bg-[#1c43a6] text-white ring-4 ring-[#1c43a6]/20'
                    ...
                    <span className={`text-xs font-bold uppercase tracking-wider ${i <= currentStep ? 'text-[#1c43a6]' : 'text-slate-500'}`}>
                      Bước {i + 1}
                    </span>
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`h-px grow mx-4 transition-colors ${i < currentStep ? 'bg-[#1c43a6]/30' : 'bg-slate-200'}`}></div>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="p-8">
              ...
              {currentStep === 0 && (
                <div className="animate-fade-in">
                  <h2 className="text-slate-900 text-2xl font-bold mb-6 flex items-center gap-2">
                    <span className="bg-[#1c43a6]/10 text-[#1c43a6] w-8 h-8 rounded-lg flex items-center justify-center text-lg font-black">1</span>
                    ...
                    <label
                      className={`relative flex flex-col p-5 cursor-pointer rounded-xl border-2 transition-all ${
                        businessTime === 'ALL_DAY'
                          ? 'border-[#1c43a6] bg-[#1c43a6]/5'
                          : 'border-slate-200 hover:border-[#1c43a6]/50 hover:bg-slate-50'
                      }`}
                      onClick={() => setBusinessTime('ALL_DAY')}
                    >
                      <input
                        checked={businessTime === 'ALL_DAY'}
                        className="absolute top-4 right-4 h-5 w-5 text-[#1c43a6] border-slate-300 focus:ring-[#1c43a6]"
                        name="business_time"
                        type="radio"
                        onChange={() => setBusinessTime('ALL_DAY')}
                      />
                      <span className="material-symbols-outlined text-[#1c43a6] text-4xl mb-3">update</span>
              ...
              {currentStep === 1 && (
                <div className="animate-fade-in">
                  <h2 className="text-slate-900 text-2xl font-bold mb-6 flex items-center gap-2">
                    <span className="bg-[#1c43a6]/10 text-[#1c43a6] w-8 h-8 rounded-lg flex items-center justify-center text-lg font-black">2</span>
                      ...
                      <label
                        ...
                        className={`flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition-all ${
                          selectedChannels.includes(ch.id)
                            ? 'border-[#1c43a6] bg-[#1c43a6]/5'
                            : 'border-slate-200 bg-slate-50 hover:border-[#1c43a6]/40'
                        }`}
                      >
                        <input
                          checked={selectedChannels.includes(ch.id)}
                          className="rounded text-[#1c43a6] focus:ring-[#1c43a6] h-5 w-5"
                          type="checkbox"
              ...
              <div className="flex items-center justify-between pt-6 border-t border-slate-100">
                ...
                {currentStep < 2 ? (
                  <button
                    ...
                    className="px-10 py-3 bg-[#1c43a6] text-white font-bold rounded-lg shadow-lg shadow-[#1c43a6]/30 hover:shadow-[#1c43a6]/40 hover:-translate-y-0.5 transition-all flex items-center gap-2"
                  >
                    Tiếp theo
      ...
      <footer className="bg-white border-t border-slate-200 py-6 px-6 md:px-40">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            <a className="text-slate-400 hover:text-[#1c43a6] text-sm transition-colors" href="#">Điều khoản dịch vụ</a>
            <a className="text-slate-400 hover:text-[#1c43a6] text-sm transition-colors" href="#">Chính sách bảo mật</a>
          </div>
        </div>
      </footer>
    </div>
